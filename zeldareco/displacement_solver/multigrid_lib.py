...
def jacobi_jit(v: np.ndarray, f: np.ndarray, jac_buffer: np.ndarray, nmesh_dims: np.ndarray, 
    ...
    """
    Esegue un'iterazione di Jacobi sul reticolo per l'equazione differenziale modificata.
    \[ \nabla \cdot \left( \mathbf{G} \cdot \nabla v \right) = f \]

    Parameters:
    -----------
    v : array 1D float
        Griglia di input/output (flattened).
    f : array 1D float
        Griglia sorgente (flattened). In our case, f = -delta.
    jac_buffer : array 1D float
        Buffer temporaneo per i risultati (flattened).
    nmesh_dims : array-like
        Dimensioni globali della griglia [Nx, Ny, Nz].
    localnmeshx : int
        Dimensione X locale della fetta (per MPI).
    offsetx : int
        Offset globale X della fetta (per MPI).
    boxsize : array-like
        Dimensioni fisiche della scatola [Lx, Ly, Lz].
    boxcenter : array-like
        Centro fisico della scatola [Cx, Cy, Cz].
    beta : float
        Fattore di modifica per il tensore G.
    damping_factor : float
        Fattore di smorzamento per l'aggiornamento di Jacobi.
    los_vector : array-like
        Vettore di linea di vista (per approssimazione piano-parallelo).
    use_plane_parallel : bool
        Se True, usa l'approssimazione piano-parallelo. Altrimenti, usa coordinate reali.
    """
    ...
    for ix_local in prange(localnmeshx):
        ...
        # Logica periodica sicura (Modulo positivo)
        ...
        for iy in range(ny):
            ...
            for iz in range(nz):
                ...
                # Cross terms (they are zero if beta=0, so we can skip them in that case)
                if beta != 0.0:
                ...
                # --- PUNTO CRITICO 3: Diagonale ---
                diag = 2.0 * (gpx2 + gpy2 + gpz2)
                
                if diag == 0.0:
                    jac_buffer[ii] = 0.0 # Impossibile matematicamente, ma sicuro
                else:
                    jac_buffer[ii] = res / diag

    # UPDATE FINALE
    for i in prange(localnmeshx * stride_yz):
        # Protezione finale: se jac_buffer ha NaN, non corrompere v (per debug)
        val_new = jac_buffer[i]
        
        # Update standard
        v[i] = (1.0 - damping_factor) * v[i] + damping_factor * val_new
'''
@njit(fastmath=True, parallel=True, nogil=True, cache=True) 
def jacobi_jit(v: np.ndarray, f: np.ndarray, jac_buffer: np.ndarray, nmesh_dims: np.ndarray, 
            localnmeshx: int, offsetx: int, boxsize: np.ndarray, boxcenter: np.ndarray, 
