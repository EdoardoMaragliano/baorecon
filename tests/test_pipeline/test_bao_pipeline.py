...
from zeldareco.pipeline.bao_pipeline import ReconstructionPipeline
...
@pytest.fixture(scope="module")
def test_config_all_outputs(tmp_path_factory, dummy_catalogs):
    """Create a YAML config file that enables all save options."""
...
def test_pipeline_saves_all_outputs(test_config_all_outputs):
    """
    Run the pipeline with a config that saves all possible outputs
    and verify that all files are created correctly.
    """
