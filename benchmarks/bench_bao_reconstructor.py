...
def worker(spec):
    ...
    return _worker_baorecon(n, nmesh, device, repeats, solver, mas_parallel)
...
def main():
    ...
    if args.debug:
        if args.solver == 'multigrid':
            ...
            _worker_baorecon(n=int(1e4), nmesh=64, device="cpu", repeats=1, solver_type="multigrid", mas_parallel=False)
            return
        else:
            print("RUNNING BAORECON DIRECTLY TO SURFACE THE ERROR...")
            _worker_baorecon(n=int(1e4), nmesh=64, device="cpu", repeats=1, solver_type="ifft", mas_parallel=False)
            return
