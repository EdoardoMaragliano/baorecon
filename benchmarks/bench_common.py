...
* a single timing+memory measurement primitive that honours the project's
  benchmarking conventions (Numba warmup, peak resident set size via
  ``resource.getrusage`` for memory),
...
Nothing here imports or mutates the ``zeldareco`` package; the benchmark scripts
...
#   memory_peak_mb  -- memory attributable to the step (RSS delta)
#   memory_total_mb -- absolute peak of the whole process (RSS)
...
# zeldareco even from a plain checkout that was never ``pip install``-ed.
...
# Process peak-RSS baseline (KiB) against which memory deltas are reported.
...
def measure(fn: Callable[[], object], repeats: int = 5, warmup: bool = True) -> Measurement:
    """Time ``fn`` ``repeats`` times and measure its peak memory.

    * ``fn`` takes no arguments and returns its result.
    * A warmup call is issued first so Numba JIT compilation is not counted in the
      timings.
    * Memory comes from the process peak resident set size,
...
def safe_measure(fn: Callable[[], object], *, label: str, repeats: int = 5,
                 warmup: bool = True) -> Optional[Measurement]:
    """Like :func:`measure` but converts failures (e.g. OOM) into a skip + ``None``.

    This keeps a single heavy configuration (a huge mesh that exhausts RAM) from
    aborting the whole sweep. Note Numba kernels are JIT-compiled on the per-call
    warmup inside :func:`measure`; in the per-backend worker process this fully
    primes the exact kernels that configuration uses, so no separate global warmup
    is needed.
    """
    try:
        return measure(fn, repeats=repeats, warmup=warmup)
...
# workers and aggregates their JSON output; it never imports zeldareco/pyrecon.
WORKER_FLAG = "--worker"
